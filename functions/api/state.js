...
async function buildCapabilities(request, env) {
  const db = getDB(env);
  const bindingOk = isD1Binding(db);
  const legacy = await getLegacyCapabilities(env).catch(() => ({ ok: true, available: true, remote_sync_allowed: bindingOk }));
  return buildJsonResponse(request, {
    ok: true,
    available: true,
    local_static_mode: !!legacy.local_static_mode,
    remote_sync_allowed: !!(bindingOk || legacy.remote_sync_allowed),
...
async function handlePull(request, env, tenant) {
  try {
    ...
    return syncJson({ ok: false, error: 'state_pull_failed', detail: String(err && (err.message || err) || 'unknown_error') }, 500, request);
  }
}
...
async function handlePush(request, env, tenant) {
  try {
    ...
    return syncJson({ ok: false, error: 'state_push_failed', detail: String(err && (err.message || err) || 'unknown_error') }, 500, request);
  }
}
