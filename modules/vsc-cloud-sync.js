(() => {
  ...
  const META_KEY = 'vsc_cloud_sync_meta_v2';
  ...
  function apiUrl(action, extra = ''){ return `${apiBase()}/api/state?action=${encodeURIComponent(action)}${extra}`; }
  ...
  async function getCapabilities(force){
    ...
    const r = await apiJSON(apiUrl('capabilities'), { method:'GET', headers:{ 'Accept':'application/json', 'X-VSC-Tenant': getTenantKey() } });
  ...
  async function pushSnapshot(){
    ...
    const r = await apiJSON(apiUrl('push'), {
      method:'POST',
      headers: buildHeaders(),
      body: JSON.stringify({ snapshot, source: isLocalDev() ? 'local-manual-sync' : 'web-manual-sync' }),
    });
    if (!r.ok || !r.json || !r.json.ok) throw new Error((r.json && r.json.error) || `push_failed_${r.status}`);
    state.lastPushAt = Date.now();
    setSavedMeta(Object.assign({}, getSavedMeta(), { tenant:getTenantKey(), last_pushed_at: nowIso(), meta:r.json.meta || null }));
    return r.json;
  }

  async function pullSnapshot(){
    const tenant = encodeURIComponent(getTenantKey());
    const r = await apiJSON(apiUrl('pull', `&tenant=${tenant}`), { method:'GET', headers:{ 'Accept':'application/json', 'X-VSC-Tenant': getTenantKey() } });
  ...
  async function manualSync(){
    ...
    try {
      ...
      emit({ running:false, pending:0, ok:true, manual:true, lastError:null, local_static_mode:isLocalDev(), remote_sync_allowed:true });
  ...
  window.VSC_CLOUD_SYNC = {
    ...
    async pullNow(){ await ensureCoreReady(); await getCapabilities(true); return pullSnapshot(); },
