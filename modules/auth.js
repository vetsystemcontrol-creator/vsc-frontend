  ...
  try{
    ...
    if(!legacyStore){
      const guess = names.find((n) => {
        return /(^|_)(users?|usuarios?)(_|$)/i.test(String(n||""));
      });
      if(guess && !CANONICAL_BLOCKLIST.has(String(guess))){
        const hasRows = await sampleStoreHasUserRows(String(guess));
        if(hasRows) legacyStore = String(guess);
      }
    }
...
})(); // Fechamento da IIFE principal (window.VSC_AUTH)
