...
req.onupgradeneeded = (e) => {
  ...
  async function outboxEnqueue(entity, action, entity_id, payload){
    ...
    // SGQT 8.6 - Auditoria de Exclusão (Enterprise)
    // Se a ação for DELETE, registra no log de auditoria de negócios.
    const isDelete = String(action).toUpperCase() === "DELETE";
    const stores = isDelete ? [STORE_OUTBOX, STORE_BUSINESS_AUDIT] : [STORE_OUTBOX];
      ...
      if (isDelete) {
        try {
          const audit = {
            ...
            entity_id: String(entity_id || ""),
            action: "DELETE",
            user_id: vscCurrentUserId(),
            payload_summary: payload ? JSON.stringify(payload).slice(0, 200) : ""
          };
          s[STORE_BUSINESS_AUDIT].add(audit);
        } catch (_) {}
      }
    });
